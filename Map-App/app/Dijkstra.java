...
public class Dijkstra {
    ...
    public static void findShortestPath(Digraph graph, Vertex source, Vertex destination) {
        ...
        // Find the vertex with the least distance to reach.
        Vertex minNode = findVertexWithMinDist();
            ...
            for (Edge edge : edges) {
                ...
                if (NOT_VISITED.contains(edge.getDestination())) {
                    ...
                    if (alt < DISTANCE_MAP.get(edge.getDestination())) { // If the alternative cost is smaller than the current cost.
                        // Updates the min cost to reach this vertex.
                        DISTANCE_MAP.put(edge.getDestination(), alt);

                        // Updates the previous vertex to reach this current vertex.
    ...
    {
        // Using the preceding Vertex to re-create path to your target/destination.
    ...
    private static Vertex findVertexWithMinDist()
    {
        // Using linear search to search for the min cost vertex based on the distance.
        Vertex minNode = null;
